 ...
 */
export function discoverItems(items: DiscoveryItem[], currentId: string, now: Date) {
  ...
  const candidates = items.filter((item) => item.id !== currentId && item.growthStatus !== "published")
